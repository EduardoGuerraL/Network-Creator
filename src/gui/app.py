...
def show_initial_splash():
    ...
    def draw_network(surface, alpha_fade):
        ...
        for node in nodes:
            ...
            if node[0] < 0 or node[0] > splash_size[0]: node[2] *= -1
            if node[1] < 0 or node[1] > splash_size[1]: node[3] *= -1
    ...
    # Configuramos una ventana pequeña, sin bordes (NOFRAME) y centrada
    splash_size = (1000, 400)
    os.environ['SDL_VIDEO_CENTERED'] = '1' # Centra la ventana en Ubuntu
  ...
  # --- Paleta de colores Deep Tech ---
    ...
    text_rect = text_surf.get_rect(center=(splash_size[0]//2, splash_size[1]//2 - 10))
    ...
    subtitle_rect = subtitle_surf.get_rect(center=(splash_size[0]//2, splash_size[1]//2 + 45))
    
    # Creamos nodos flotantes en posiciones aleatorias
    N = 50
    nodes = [[random.randint(0, splash_size[0]), random.randint(0, splash_size[1]), 
    ...
    for alpha in range(0, 255, 2): # Aparece
        ...
        temp_surface = pygame.Surface(splash_size, pygame.SRCALPHA)
    ...
    pygame.display.quit() # Cerramos esta mini-ventana temporal